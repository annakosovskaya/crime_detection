    ...
    def __getitem__(self, idx):
        if self.is_train == 1:
            ...
            try:
                ...
                print(f"Warning: Could not find feature file for {name}. Skipping.")
...
class Anomaly_Loader(Dataset):
    ...
    def __getitem__(self, idx):
        if self.is_train == 1:
            ...
            concat_npy = np.concatenate([rgb_npy, flow_npy], axis=1)
            if self.modality == 'RGB':
                return rgb_npy, gts, frames, category
            elif self.modality == 'FLOW':
                return flow_npy, gts, frames, category
            else:
                return concat_npy, gts, frames, category
